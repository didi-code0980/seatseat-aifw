...
| ID | Status | Title | Description | Group | Invariants touched | Notes |
|----|--------|-------|-------------|-------|--------------------|-------|
| REG-01 | PLANNED | User self-release | An occupant releases the seat they currently occupy, without approval, taking effect immediately. | REG | [] | **🟡 REMOVED 2026-08-27. The operator answered the question it was waiting on: split.** Their words: *"tách REG-01 và REG-02."* This row keeps self-release; the request workflow moves to `REG-02`. **The `Title` and `Description` are narrowed, not rewritten.** The old title, *"User self-release and seat request workflow"*, came from a `ticket.yaml` seeded outside the loop on 2026-08-25, and the compound was itself the unstated assumption the 🟡 named — the backlog has carried *"User self-release"* alone since Phase C. `Description` is transcribed from `glossary.md` §Self-release, which is registry text and not composed here. **The split is the glossary's own division**: §Self-release takes effect *without approval*, §Request goes *"to a Manager or Admin for approval"* — two entries, opposite on the one axis that decides the surface. `Invariants touched` stays `[]`: the seeded ticket's four IDs had no source and were removed in PR #56, and SPEC determines the real set. INV-06 is the likely one — §Self-release says an occupant exit auto-downgrades the seat's primary device — but likely is not the same as determined. **Not startable yet, and not for a scope reason:** §Self-release is *"an occupant vacating a seat they currently occupy"*, and nothing in `src/` resolves the signed-in Member — no `getCurrentMember`, no `authUserId`, and `PermissionGate` is imported nowhere. `SYS-02` adds `Account.auth_user_id` and is in flight. |
| REG-02 | PLANNED | Seat request workflow | A Member requests a seat — targeted, naming one seat, or open, naming only a room — and a Manager or Admin approves it. | REG | [] | **Written 2026-08-27 by the orchestrator, decided by the operator:** *"tách REG-01 và REG-02."* Carries the half `REG-01` gave up. **`REG-02` is the next free number in this group and no number is reused.** `Title` and `Description` are transcribed from `glossary.md` §Request (seat request) — *"A Member asking to occupy a seat"*, the Targeted and Open forms, and *"Both forms go to a Manager or Admin for approval"* — because the operator named the split rather than the content, and the glossary is registry text rather than something composed here. **No 🟡**: the scope is not an assumption an agent made, it is the answer to the question the 🟡 asked. `Invariants touched` is `[]` — the operator named none and SPEC determines the real set. **Two things this row does not decide and SPEC must not invent**: whether approval is a rank check or a stored request record with a state, and what happens to a request whose seat is taken before it is approved. Neither is answered anywhere in `.ai/registry/**`. **It needs the same thing `REG-01` needs and needs more of it** — a Member identity for the requester *and* a rank for the approver, and INV-08's rank enforcement is held today by a browser-storage flag that enforces nothing (MD-14). |
