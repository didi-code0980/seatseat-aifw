...
**This command writes an artifact and it has a gate** — it amends the idea file, and the TRIAGE row of
the stage ownership table in `.ai/01-operating-model.md` reads *verdict is one of REJECT, NEEDS-ADR,
PROMOTE, with a reason*. Quote that verdict on the first line; `gate n/a` is wrong here and was written
into this file by mistake on 2026-08-25 (MD-35). *Tiếp theo* names
